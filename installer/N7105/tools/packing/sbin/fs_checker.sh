...
###### AGNi FS CHECKER v1.3 (I9305 N710x I605)################

SYSTEM_PARTITION="mmcblk0p13"
DATA_PARTITION="mmcblk0p16"
CACHE_PARTITION="mmcblk0p12"
PRELOAD_PARTITION="mmcblk0p14"
...
if [ "$SYSTEM_FS" == "EXT4" ];
	then
	$BBOX awk '/AGNI_SYSTEM/{print;print "/dev/block/mmcblk0p13		/system			ext4		ro,noatime		wait";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/system entry as EXT4 added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$SYSTEM_FS" == "F2FS" ];
	then
	$BBOX awk '/AGNI_SYSTEM/{print;print "/dev/block/mmcblk0p13		/system			f2fs		ro,noatime,background_gc=off	wait";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/system entry as F2FS added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$CACHE_FS" == "EXT4" ];
	then
	$BBOX awk '/AGNI_CACHE/{print;print "/dev/block/mmcblk0p12		/cache			ext4		noatime,nosuid,nodev,journal_async_commit,errors=panic		wait";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/cache entry as EXT4 added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$CACHE_FS" == "F2FS" ];
	then
	$BBOX awk '/AGNI_CACHE/{print;print "/dev/block/mmcblk0p12		/cache			f2fs		noatime,nosuid,nodev,background_gc=off		wait";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/cache entry as F2FS added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$PRELOAD_FS" == "EXT4" ];
	then
	$BBOX awk '/AGNI_PRELOAD/{print;print "/dev/block/mmcblk0p14		/preload		ext4		noatime,nosuid,nodev,journal_async_commit		wait";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/preload entry as EXT4 added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$PRELOAD_FS" == "F2FS" ];
	then
	$BBOX awk '/AGNI_PRELOAD/{print;print "/dev/block/mmcblk0p14		/preload		f2fs		noatime,nosuid,nodev,background_gc=off		wait";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/preload entry as F2FS added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$DATA_FS" == "EXT4" ];
	then
	$BBOX awk '/AGNI_DATA/{print;print "/dev/block/mmcblk0p16		/data			ext4		noatime,nosuid,nodev,discard,noauto_da_alloc,journal_async_commit,errors=panic		wait,check,encryptable=footer";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
	$BBOX echo "/data entry as EXT4 added to /fstab.smdk4x12" >> $TMP_LOG_FILE
fi
if [ "$DATA_FS" == "F2FS" ];
	then
	$BBOX awk '/AGNI_DATA/{print;print "/dev/block/mmcblk0p16		/data			f2fs		noatime,nosuid,nodev,discard,background_gc=off	wait,encryptable=footer";next}1' /fstab.smdk4x12 > /fstab.smdk4x12-temp
	$BBOX mv /fstab.smdk4x12-temp /fstab.smdk4x12
